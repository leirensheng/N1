 ...
 */
  ...
  async requestFile(url, fileName) {
    ...
    const remoteStream = request(url);
    const stream = fs.createWriteStream(fileName);
    await new Promise(resolve => {
      remoteStream.pipe(stream);
  ...
  getWhere(queryItems) {
    return queryItems.filter(one => one.value !== '').reduce((prev, cur) => {
      const { column, exp, value } = cur;
      const val = exp === 'like' ? `%${value}%` : value;
      prev[column] = {
        [Op[exp]]: val,
      };
      return prev;
